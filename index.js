...
// ========== SHOP ROUTES ==========
// ============================================
// 🔥 SHOP ROUTES WITH SLUG SUPPORT
// ============================================

// Helper function to find shop by ID or slug
async function findShopByIdentifier(identifier) {
    const isObjectId = /^[0-9a-fA-F]{24}$/.test(identifier);
    
    if (isObjectId) {
        return await Singup.findById(identifier);
    } else {
        return await Singup.findOne({ slug: identifier });
    }
}

// 🏪 GET SHOP BY SLUG OR ID (PUBLIC - No Auth Required)
app.get("/shop/:identifier", async (req, res) => {
    try {
        ...
        if (/^[0-9a-fA-F]{24}$/.test(identifier) && shop.slug) {
            return res.redirect(`/shop/${shop.slug}`);
        }
...
// 🏪 GET SHOP WITH PRODUCTS (PUBLIC)
app.get("/shop/:identifier/details", async (req, res) => {
    try {
        const { identifier } = req.params;
        const shop = await findShopByIdentifier(identifier);
...
app.get("/shops", async (req, res) => {
    try {
        const shops = await Singup.find()
            .select('shopName slug image description email phone address')
            .sort({ shopName: 1 });
...
app.get("/my-shop", auth, async (req, res) => {
    try {
        const shop = await Singup.findById(req.userId);
        ...
        res.json({
            ...
            shop: {
                ...
                heroPublicId: shop.heroPublicId
            }
        });
...
app.post("/addproduct", auth, upload.single('image'), async (req, res) => {
    try {
        ...
        const shop = await Singup.findById(req.userId).select('slug shopName');
        ...
        res.json({
            ...
            product: {
                ...
                shareableLink: `/product/${shop?.slug || req.userId}/${product.slug || product._id}` // ✅ Clean URL
            }
        });
...
async function findProductByIdentifier(identifier) {
    ...
    if (isObjectId) {
        return await Product.findById(identifier).populate('owner', 'shopName slug email');
    } else {
        return await Product.findOne({ slug: identifier }).populate('owner', 'shopName slug email');
    }
}
...
app.get("/product/:identifier", async (req, res) => {
    try {
        ...
        // If accessed by ID, redirect to slug URL
        if (/^[0-9a-fA-F]{24}$/.test(identifier) && product.slug) {
            const shopSlug = product.owner?.slug || product.owner?._id;
            return res.redirect(`/product/${shopSlug}/${product.slug}`);
        }
        ...
        res.json({
            ...
            product: {
                ...
                owner: {
                    ...
                    shopName: product.owner?.shopName || 'Unknown',
                    shopSlug: product.owner?.slug || ''
                },
                stats: {
                    averageRating: averageRating || 0,
...
// ========== GET PRODUCT BY SHOP SLUG + PRODUCT SLUG ==========
app.get("/product/:shopIdentifier/:productIdentifier", async (req, res) => {
    try {
        const { shopIdentifier, productIdentifier } = req.params;
        
        // Find shop first
        let shop;
        const isShopObjectId = /^[0-9a-fA-F]{24}$/.test(shopIdentifier);
        
        if (isShopObjectId) {
            shop = await Singup.findById(shopIdentifier);
        } else {
            shop = await Singup.findOne({ slug: shopIdentifier });
        }
        ...
        // Find product
        let product;
        const isProductObjectId = /^[0-9a-fA-F]{24}$/.test(productIdentifier);
        
        if (isProductObjectId) {
            product = await Product.findById(productIdentifier).populate('owner', 'shopName slug');
        } else {
            product = await Product.findOne({ 
                slug: productIdentifier,
                owner: shop._id 
            }).populate('owner', 'shopName slug');
        }
        ...
        // Redirect to clean slug URL if needed
        if (isProductObjectId && product.slug) {
            return res.redirect(`/product/${shop.slug || shop._id}/${product.slug}`);
        }
...
// ========== GET SHOP PRODUCTS (Updated with slugs) ==========
app.get("/shop-products/:identifier", async (req, res) => {
    try {
        const { identifier } = req.params;
        ...
        if (isObjectId) {
            shop = await Singup.findById(identifier);
        } else {
            shop = await Singup.findOne({ slug: identifier });
        }
        ...
        const products = await Product.find({ owner: shop._id })
            .select('name slug price description image averageRating totalReviews');
        ...
        res.json({
            ...
            products: products.map(p => ({
                ...p.toObject(),
                shareableLink: `/product/${shop.slug || shop._id}/${p.slug || p._id}`
            }))
        });
